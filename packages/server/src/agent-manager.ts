...
export class AgentManager {
  ...
  constructor(
    ...
    // The `reload_config` tool (Acme's, after it edits config on disk) applies
    // every surface in one explicit shot. Nothing watches the filesystem.
    bindReloadConfig(() => this.reloadAll());
  ...
  /**
   * Apply every config surface from disk in one shot, then evict cached
   * Agents so the next turn rebuilds with the fresh state. Nothing watches
   * the filesystem — this is the single, explicit "apply my edits" path,
   * invoked by the `reload_config` tool after an agent edits config files
   * (config.yaml, AGENTS.md, mcp.json, coworkers' AGENT.md, skills).
   *
   * Deliberately full + explicit: MCP reinit (disconnect + reconnect servers
   * + restart workers) is expensive, so it must never fire automatically on a
   * file change — only here, when an agent decides its edits are done. Runs in
   * the daemon (the tool is daemon-runtime), so restarting workers doesn't
   * kill the calling agent's own turn.
   *
   * Returns what still genuinely needs a process restart (server host/port —
   * a live socket bind that can't be swapped in place).
   */
  async reloadAll(): Promise<{ restartRequired: string[] }> {
