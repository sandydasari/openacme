...
There's a fourth, and in dollar terms it's the sharpest. The big model providers cache the front of your prompt now, the standing instructions plus the conversation so far, and a call that starts with the exact same text as the last one is billed at a fraction for that shared part. The catch is it has to be identical from the very start. That rewards leaving the prompt alone between calls and punishes rebuilding it. Reacting invites a rebuild. You wake because something changed, so the natural move is to fold that change into the agent's context, and the front of the prompt shifts, so you pay full freight on every wake. The loop runs the agent on the same standing context wake after wake, so the front matches what you sent last time and the discount holds. You know the shape from a database that's only fast once its cache is warm. Keep thrashing it and it never warms up.

Three of those four have nothing to do with AI, which is the point. Expensive, one writer, no rush, wherever those line up, reacting to every change is the wrong reflex, whatever the work is. The caching one needs the work to be a model. The other three don't care what the work is.
