...
And this isn't a case against event-driven in general. Polling is usually the worse option, wasteful, a bit laggy, the thing you're taught to grow out of. This is only about the narrow kind of work where that flips.
...
So I fixed them, all of them. And then I wrote a watchdog: if an agent woke a few times in a row and found nothing to do, it would step in and calm it down, because the event system could fire an agent at nothing, over and over, burning money on calls that did nothing. I had written a program whose whole job was to babysit my scheduler and protect me from it. That's a confession, not a feature.
...
But look at the trade. I gave up instant reactions and a class of silent, unbounded, expensive failures, and got back failures that are loud, bounded, and cheap. A pointless wake costs nothing and shows up on a graph. A lost wake costs you the work and shows up nowhere. I'll take the first one every time.
