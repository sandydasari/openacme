...
- **System-prompt cache invalidation is manual.** Changing an agent's tools / skills mid-process won't take effect until the cached `Agent` is evicted (`agents.clear()`/`delete(id)`) or you call `invalidateSystemPromptCache()`. AgentManager evicts on agent-definition mutation, and the config watcher evicts on a matching on-disk edit (below).
- **Config watcher = no restart for file edits.** `AgentManager.startConfigWatcher()` (chokidar, started at end of constructor) watches the dataDir root and, on an edit to `config.yaml` / `AGENTS.md` / `mcp.json` / `agents/<id>/AGENT.md` / `skills/**/SKILL.md`, runs the matching reload primitive (`reloadConfig` / re-read+evict / `initMCP` / evict+conditional reinit / `reloadSkills`). Noisy subtrees (workspace, memory, tasks, attachments, browser-profiles, sessions, `*.db`) are pruned; 250ms debounce + an 800ms self-write suppression window (`suppressWatch`) keeps the manager's own writes from double-reloading. The only edit still needing `openacme restart` is `server.host`/`server.port` (live socket bind). Handlers swallow errors (a malformed file logs a warn, never crashes).
